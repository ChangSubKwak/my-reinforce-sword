...
- **렌더 모델**: 매 액션 후 `save()` → `render()` 풀 리렌더. 부분 업데이트 없음. **안전망 (v400)**: `save()`가 `stateSeq++`, `render()` 말미가 `renderedSeq = stateSeq` — 1초 인터벌이 어긋나면 그린다. *타이머가* 상태를 바꾸는 경로(연일·정진·명절·첨·삼례)가 `save()`로만 끝나 화면이 고착되던 것을 진원에서 막는다. **`render()` 안에서 `save()` 를 부르면 순환** (테스트 잠금).
...
| 16 | MED | **타이머가 바꾼 상태가 화면에 닿지 않았다** — 렌더 모델은 `save()` → `render()` 인데 시간 보상 5경로(연일 마일스톤·정진·명절·첨·삼례)가 `save()`로 끝났다. 유일한 주기적 `render()`인 呼吸 tier 인터벌은 tier가 고정되면 발화하지 않아, 밤새 켜둔 탭(=시간 보상이 도착하는 바로 그 상황)에서 영구히 낡은 값이 남았다. 실측: 자정 보상 +30 으로 state 35 인데 상단 카운터는 5, 강화 버튼은 「조각 부족 (8 필요 / 보유 5)」로 **disabled 인 채 굳어 마우스·스페이스·엔터 모두 먹지 않았다** — 조각을 쥐고도 강화할 수 없다. → 개별 호출부를 늘리는 대신(그렇게 다섯이 쌓였다) `stateSeq`/`renderedSeq` 버전 대조 + 1초 안전망. 실측 발화율은 저장의 13%이고 `render(); save();` 순서는 0곳이라 헛발화가 아니다 |
...
**규율 (v400 추가)**
- **상태를 바꾸는 주체가 사용자만은 아니다.** 액션 경로만 렌더 규율을 지키면, 타이머가 바꾼 상태는 화면에 닿지 못한다 ([16]). 호출부를 하나씩 늘리는 방식은 누락이 쌓인다 — 진원에서 대조할 것.
